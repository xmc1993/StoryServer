...
public class UserReviewController {
	...
	@ResponseBody
	public ResponseData<Boolean> saveReview(@ApiParam("workId") @RequestParam("workId") int workId,
			...
			@ApiParam("content") @RequestParam("content") String content, HttpServletRequest request,
			HttpServletResponse response) {
		if (workService.getWorksById(workId) == null)
			throw new RuntimeException("错误的workId");
		ResponseData<Boolean> result = new ResponseData<>();
		if (parentId != null && reviewService.getReviewById(parentId) == null)
			throw new RuntimeException("错误的parentId");
		if (userService.getUserById(fromUserId) == null)
			throw new RuntimeException("错误的评论用户ID");
		if (toUserId != null && userService.getUserById(toUserId) == null)
			throw new RuntimeException("错误的回复用户ID");
	...
	public ResponseData<Boolean> updateReview(@RequestParam("reviewId") int reviewId,
			@RequestParam("fromUserId") int fromUserId, @RequestParam("content") String content) {
		ResponseData<Boolean> result = new ResponseData<>();
	...
	public ResponseData<Boolean> deleteReview(@RequestParam("reviewId") int reviewId,
		...
		if (review == null)
			throw new RuntimeException("错误的评论Id");
		Boolean temp = null;
		if (review.getParentId() == 0 && review.getFromUserId() == userId) {
			temp = reviewService.deleteReviewByUser(reviewId, userId);
		} else if (review.getParentId() != 0
				&& reviewService.getReviewById(review.getParentId()).getFromUserId() == userId) {
		...
		if (temp == null)
			throw new RuntimeException("错误的删除");
		else if (temp == true) {
			result.jsonFill(1, null, true);
		} else {
			result.jsonFill(2, null, false);
